...
pub mod vectorize {
    ...
    /// Serialize a type that implements IntoIterator as a sequence.
    pub fn serialize<I, S>(iter: I, serializer: S) -> Result<S::Ok, S::Error>
    where
        I: IntoIterator,
    ...
    /// Deserialize a list of pairs into something that implements `FromIter<K, V>`
    pub fn deserialize<'de, C, K, V, D>(deserializer: D) -> Result<C, D::Error>
    where
        K: Deserialize<'de>,
        V: Deserialize<'de>,
        C: FromIterator<(K, V)>,
        D: Deserializer<'de>,
    {
        Vec::<(K, V)>::deserialize(deserializer).map(|v| v.into_iter().collect())
    }
}
...
pub mod prefixmap {
    ...
    /// Serialize a type that implements IntoIterator as a sequence.
    pub fn serialize<PM, T, S>(map: &PM, serializer: S) -> Result<S::Ok, S::Error>
    where
        PM: PrefixMap<T>,
        T: Serialize,
        S: Serializer,
    ...
    /// Deserialize a list of pairs into something that implements `FromIter<K, V>`
    pub fn deserialize<'de, PM, T, D>(deserializer: D) -> Result<PM, D::Error>
    where
        PM: PrefixMap<T>,
        T: Deserialize<'de>,
        D: Deserializer<'de>,
    {
        Vec::<(PM::P, T)>::deserialize(deserializer).map(|v| v.into_iter().collect())
    }
}

pub mod prefixmap_of_map {
    use crate::types::PrefixMap;
    use serde::{Deserialize, Deserializer, Serialize, Serializer};
    ...
    /// Serialize a type that implements IntoIterator as a sequence.
    pub fn serialize<PM, K, V, S>(map: &PM, serializer: S) -> Result<S::Ok, S::Error>
    where
        PM: PrefixMap<HashMap<K, V>>,
        K: Serialize,
        V: Serialize,
        S: Serializer,
    {
        serializer.collect_seq(
            map.iter()
                .map(|(p, x)| (p, x.into_iter().collect::<Vec<_>>())),
        )
    }

    /// Deserialize a list of pairs into something that implements `FromIter<K, V>`
    pub fn deserialize<'de, PM, K, V, D>(deserializer: D) -> Result<PM, D::Error>
    where
        PM: PrefixMap<HashMap<K, V>>,
        ...
        D: Deserializer<'de>,
    {
        Vec::<(PM::P, Vec<(K, V)>)>::deserialize(deserializer).map(|v| {
            v.into_iter()
...
pub mod prefixset {
    use crate::types::PrefixSet;
    use serde::{Deserialize, Deserializer, Serializer};

    /// Serialize a type that implements IntoIterator as a sequence.
    pub fn serialize<PS, S>(set: &PS, serializer: S) -> Result<S::Ok, S::Error>
    where
        PS: PrefixSet,
        S: Serializer,
    {
    ...
    /// Deserialize a list of pairs into something that implements `FromIter<K, V>`
    pub fn deserialize<'de, PS, D>(deserializer: D) -> Result<PS, D::Error>
    where
        PS: PrefixSet,
        D: Deserializer<'de>,
    {
        Vec::<PS::P>::deserialize(deserializer).map(|v| v.into_iter().collect())
    }
}
